# ============================================================================
# FILE: routes/cbf_analisis.py
# Blueprint analisis RMSE & MAE CBF - bar chart + tabel per metrik
# ============================================================================
...
cbf_analisis_bp = Blueprint('cbf_analisis', __name__, url_prefix='/cbf/analisis')


def dict_to_object(d):
    """Konversi dictionary ke object untuk akses atribut"""
    class DictObject:
...
@cbf_analisis_bp.route('/')
def analisis():
    if 'cbf_hasil' not in session:
        return redirect(url_for('cbf.generate_cbf'))

    # CBF hasil sudah dalam format dict (tidak di-serialize)
    hasil_raw = session['cbf_hasil']
    hasil = [dict_to_object(item) for item in hasil_raw]
...
print("[CBF Analisis Blueprint] Loaded")